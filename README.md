![](https://komarev.com/ghpvc/?username=ShubhamRaj03)

![GitHub followers](https://img.shields.io/github/followers/ShubhamRaj03?style=social)
# Hi, I'm Shubham Raj 👋

Data Science Graduate | Machine Learning Enthusiast | Computer Vision Developer
---

## 🚀 About Me

🎓 B.Tech Graduate in Computer Science and Engineering (Data Science)

💡 Passionate about building AI and Data-Driven solutions.

🔍 Interested in:
- Machine Learning
- Deep Learning
- Computer Vision
- Data Engineering
- Data Analytics

🚀 Open to opportunities in Data Science, Machine Learning, and AI.

📚 Continuously learning and building real-world AI solutions through hands-on projects.

---

## 📈 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=ShubhamRaj03&show_icons=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=ShubhamRaj03&layout=compact)
## 🛠️ Tech Stack
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=pytorch&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
### Programming Languages
- Python
- C++
- SQL

### Data Science & Machine Learning
- NumPy
- Pandas
- Scikit-Learn
- TensorFlow
- PyTorch

### Computer Vision
- OpenCV
- CNN
- EfficientNet
- ResNeXt

### Data Analytics
- Power BI
- Excel
- ETL Pipelines

### Tools
- Git
- GitHub
- Jupyter Notebook
- VS Code

---

## 🌟 Featured Projects

### 🎭 Multimodal Deepfake Detection System
A reliability-aware deepfake detection framework combining image, video, and audio analysis using deep learning models.

**Tech:** Python, TensorFlow, PyTorch, OpenCV, EfficientNet, ResNeXt

---

### 📊 NeoStats Data Engineering Project
End-to-end retail data engineering and business intelligence project including ETL pipelines, dashboards, and analytics.

**Tech:** Python, Pandas, Power BI

---

### 🤖 BankBot AI Chatbot
An AI-powered chatbot developed for answering banking-related FAQs.

**Tech:** Python, NLP

---

### 🖼️ Image Classification
Deep learning-based image classification project using CNN architectures.

**Tech:** TensorFlow, Python

---

### 💈 Barber Appointment Website
Responsive appointment booking website for barber shop management.

**Tech:** HTML, CSS, JavaScript

---

### 🌐 Personal Portfolio
Personal portfolio website showcasing projects, skills, and achievements.

**Tech:** HTML, CSS, JavaScript

---


---

## 🎯 Current Focus

- Deepfake Detection
- Computer Vision
- Data Engineering
- Machine Learning Research
- Large Scale Data Analytics

---

## 📫 Connect With Me

- LinkedIn: https://www.linkedin.com/in/shubham-raj-19403a252/
- Email: shubhamrajbju12@gmail.com

---

⭐ Passionate about leveraging AI and Data Science to solve real-world problems.
